...
@app.route('/')
def home():
    return "🤖 FREE HOSTING  🇧🇩 is Running!"
@app.route('/health')
def health():
...
# --- Configuration ---
TOKEN = '8675984061:AAGRD0BA_raVu57vxZzCTEAnGj4FfTmevaA'
OWNER_ID = 7834598531
ADMIN_ID = 7834598531
YOUR_USERNAME = '@munnabiswas6T9'
UPDATE_CHANNEL = 'https://t.me/freefiles09'
# Folder setup
...
file_size INTEGER, approved INTEGER DEFAULT 0, PRIMARY KEY (user_id, file_name))''')
...
def save_user_file_db(user_id, file_name, file_type, file_size=0, approved=0):
    """Save file info to database"""
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('''INSERT OR REPLACE INTO user_files
(user_id, file_name, file_type, upload_time, file_size, approved)
VALUES (?, ?, ?, ?, ?, ?)''',
                  (user_id, file_name, file_type, datetime.now().isoformat(), file_size, approved))
        conn.commit()
        conn.close()
...
def is_file_approved(user_id, file_name):
    """Check if file is approved by admin"""
    if user_id == OWNER_ID or user_id in admin_ids:
        return True
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('SELECT approved FROM user_files WHERE user_id = ? AND file_name = ?', (user_id, file_name))
        res = c.fetchone()
        conn.close()
...
def set_file_approved(user_id, file_name, status=1):
    """Set approval status for a file"""
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('UPDATE user_files SET approved = ? WHERE user_id = ? AND file_name = ?', (status, user_id, file_name))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"Error setting file approved: {e}")
...
def run_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🚀 <b>FREE HOSTING: STARTING SCRIPT</b> 🚀 ║
╠══════════════════════════════════════╣
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>FREE HOSTING: SCRIPT RUNNING</b> ✅   ║
╠══════════════════════════════════════╣
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>FREE HOSTING : SCRIPT FAILED</b> ❌     ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
...
def run_js_script(script_path, script_owner_id, user_folder, file_name, message_obj, attempt=1):
    ...
    try:
        ...
        terminal_msg = f"""
╔══════════════════════════════════════╗
║      🟢 <b>FREE HOSTING: STARTING NODE.JS</b> 🟢║
╠══════════════════════════════════════╣
║ 📄 File: <code>{file_name[:25]}</code>
║ 👤 User: {script_owner_id}
║ 🔄 Attempt: {attempt}/{max_attempts}
...
"""
        ...
        if process.poll() is None:
            success_msg = f"""
╔══════════════════════════════════════╗
║     ✅ <b>FREE HOSTING : NODE.JS RUNNING</b> ✅  ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ 🆔 <b>PID:</b> {process.pid}
║ ⏱️ <b>Started:</b> {datetime.now().strftime('%H:%M:%S')}
...
"""
            ...
            error_msg = f"""
╔══════════════════════════════════════╗
║     ❌ <b>FREE HOSTING : NODE.JS FAILED</b> ❌    ║
╠══════════════════════════════════════╣
║ 📄 <b>File:</b> <code>{file_name[:25]}</code>
║ ❗ <b>Exit Code:</b> {process.returncode}
╠══════════════════════════════════════╣
...
def get_file_actions_keyboard(file_name, is_running=False, approved=True):
    """Get inline keyboard for file actions"""
    markup = types.InlineKeyboardMarkup(row_width=2)
    if not approved:
        markup.add(types.InlineKeyboardButton("⏳ Pending Admin Approval", callback_data="pending_approval"))
        markup.add(
...
def stats_command(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐆𝐚𝐭𝐡𝐞𝐫𝐢𝐧𝐠 FREE HOSTING 𝐬𝐭𝐚𝐭𝐬...", duration=2)
...
def speed_command(message):
    ...
    msg = send_spinner_animation(message.chat.id, "𝐓𝐞𝐬𝐭𝐢𝐧𝐠 FREE HOSTING 𝐬𝐩𝐞𝐞𝐝...", duration=2)
...
def running_command(message):
    """Show running bots"""
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐅𝐞𝐭𝐜𝐡𝐢𝐧𝐠 FREE HOSTING 𝐛𝐨𝐭𝐬...", duration=1)
    running_bots = []
...
def process_broadcast(message):
    ...
    if not broadcast_text:
        ...
        return
    progress_msg = bot.send_message(message.chat.id, "📢 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 FREE HOSTING 𝐛𝐫𝐨𝐚𝐝𝐜𝐚𝐬𝐭...")
    success = 0
    failed = 0
...
"""
    ...
    result_text = f"""
╔══════════════════════════════════════╗
║     ✅ <b>FREE HOSTING 𝐁𝐑𝐎𝐀𝐃𝐂𝐀𝐒𝐓 𝐂𝐎𝐌𝐏𝐋𝐄𝐓𝐄</b> ✅ ║
╠══════════════════════════════════════╣
║
║  📤 𝐓𝐨𝐭𝐚𝐥: {total}
║  ✅ 𝐒𝐮𝐜𝐜𝐞𝐬𝐬: {success}
...
"""
    ...
    try:
        bot.send_message(target_user, f"🎉 𝐘𝐨𝐮'𝐯𝐞 𝐛𝐞𝐞𝐧 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐛𝐞𝐝 𝐟𝐨𝐫 {days} 𝐝𝐚𝐲𝐬 𝐛𝐲 FREE HOSTING")
    except:
        pass
...
def handle_text(message):
    ...
    if text == "📢 Updates Channel":
        bot.send_message(message.chat.id, f"📢 𝐉𝐨𝐢𝐧 𝐨𝐮𝐫 FREE HOSTING 𝐮𝐩𝐝𝐚𝐭𝐞𝐬:\n{UPDATE_CHANNEL}")
...
def show_user_files(message):
    ...
    user_id = message.from_user.id
    msg = send_spinner_animation(message.chat.id, "𝐋𝐨𝐚𝐝𝐢𝐧𝐠 FREE HOSTING 𝐟𝐢𝐥𝐞𝐬...", duration=1)
    files = user_files.get(user_id, [])
    if not files:
        text = """
╔══════════════════════════════════════╗
║       📂 <b>FREE HOSTING: YOUR FILES</b> 📂   ║
╠══════════════════════════════════════╣
║
...
"""
    ...
    for i, (file_name, file_type) in enumerate(files, 1):
        ...
        approved = is_file_approved(user_id, file_name)
        status = "🟢" if is_running else ("⏳" if not approved else "🔴")
...
def show_subscriptions(message):
    ...
    user_id = message.from_user.id
    if user_id != OWNER_ID and user_id != ADMIN_ID and user_id not in admin_ids:
    ...
    text = f"""
╔══════════════════════════════════════╗
║     💳 <b>FREE HOSTING: SUBSCRIPTIONS</b> 💳    ║
╠══════════════════════════════════════╣
║
...
"""
    ...
    markup.add(
        types.InlineKeyboardButton("📥 𝐏𝐞𝐧𝐝𝐢𝐧𝐠 𝐅𝐢𝐥𝐞𝐬", callback_data="admin_pending_files")
    )
...
"""
    ...
    try:
        ...
        is_admin_uploader = (user_id == OWNER_ID or user_id in admin_ids)
        initial_approval = 1 if is_admin_uploader else 0
                        ...
                        for f in files:
                            if f.endswith(('.py', '.js')):
                                ...
                                if (f, f.split('.')[-1]) not in user_files[user_id]:
                                    ...
                                    save_user_file_db(user_id, f, f.split('.')[-1], 0, initial_approval)
                    os.unlink(tmp_path)
                    success_text = upload_text + f"""║  ✅ 𝐙𝐈𝐏 𝐄𝐱𝐭𝐫𝐚𝐜𝐭𝐞𝐝!
║  📁 𝐅𝐢𝐥𝐞𝐬: {len(extracted_files)}
║  📌 𝐒𝐭𝐚𝐭𝐮𝐬: {'✅ 𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝' if initial_approval else '⏳ 𝐏𝐞𝐧𝐝𝐢𝐧𝐠 𝐀𝐩𝐩𝐫𝐨𝐯𝐚𝐥'}
╚══════════════════════════════════════╝
"""
            ...
            save_user_file_db(user_id, file_name, file_ext, file_size, initial_approval)
            success_text = upload_text + f"""║  ✅ 𝐔𝐩𝐥𝐨𝐚𝐝 𝐂𝐨𝐦𝐩𝐥𝐞𝐭𝐞!
║  📌 𝐒𝐭𝐚𝐭𝐮𝐬: {'✅ 𝐀𝐩𝐩𝐫𝐨𝐯𝐞𝐝' if initial_approval else '⏳ 𝐏𝐞𝐧𝐝𝐢𝐧𝐠 𝐀𝐩𝐩𝐫𝐨𝐯𝐚𝐥'}
╚══════════════════════════════════════╝
"""

        # Notify admins if it needs approval
        if not initial_approval:
            for admin_id_val in admin_ids:
                try:
                    admin_markup = types.InlineKeyboardMarkup(row_width=2)
                    admin_markup.add(
                        types.InlineKeyboardButton("✅ Approve", callback_data=f"adm_approve_{user_id}_{file_name}"),
                        types.InlineKeyboardButton("❌ Reject", callback_data=f"adm_reject_{user_id}_{file_name}")
                    )
                    bot.send_message(
                        admin_id_val,
                        f"📥 <b>New File Uploaded for Approval!</b>\n👤 User ID: <code>{user_id}</code>\n📄 File: <code>{file_name}</code>",
                        parse_mode='HTML',
        ...
        markup = types.InlineKeyboardMarkup(row_width=2)
        if file_ext in ['py', 'js'] and initial_approval:
            markup.add(
...
def handle_callback(call):
    ...
    try:
        if data.startswith("file_"):
            ...
            show_admin_pending_files(call)
        elif data.startswith("adm_approve_"):
            parts = data.split("_", 3)
            target_uid = int(parts[2])
            fname = parts[3]
            admin_approve_file(call, target_uid, fname)
        elif data.startswith("adm_reject_"):
            parts = data.split("_", 3)
            target_uid = int(parts[2])
            fname = parts[3]
            admin_reject_file(call, target_uid, fname)
        elif data == "pending_approval":
            bot.answer_callback_query(call.id, "⚠️ This file is waiting for admin approval!", show_alert=True)
...
def show_file_actions(call, file_name):
    ...
    for name, ftype in user_files.get(user_id, []):
        if name == file_name:
            ...
            break
    type_icon = "🐍" if file_type == "py" else "🟨" if file_type == "js" else "📄"
    status = "🟢 𝐑𝐮𝐧𝐧𝐢𝐧𝐠" if is_running else ("⏳ 𝐏𝐞𝐧𝐝𝐢𝐧𝐠 𝐀𝐩𝐩𝐫𝐨𝐯𝐚𝐥" if not approved else "🔴 𝐒𝐭𝐨𝐩𝐩𝐞𝐝")
    text = f"""
╔══════════════════════════════════════╗
║       📄 <b>FREE HOSTING : FILE</b> 📄         ║
╠══════════════════════════════════════╣
║
...
╚══════════════════════════════════════╝
"""
    markup = get_file_actions_keyboard(file_name, is_running, approved)
    try:
...
def run_user_script(call, file_name):
    """Run a user's script"""
    user_id = call.from_user.id
    if not is_file_approved(user_id, file_name):
        bot.answer_callback_query(call.id, "❌ File not approved yet!", show_alert=True)
        return
    user_folder = get_user_folder(user_id)
...
def show_admin_pending_files(call):
    """Show pending files for admin approval"""
    user_id = call.from_user.id
    if user_id != OWNER_ID and user_id != ADMIN_ID and user_id not in admin_ids:
    ...
    try:
        conn = sqlite3.connect(DATABASE_PATH, check_same_thread=False)
        c = conn.cursor()
        c.execute('SELECT user_id, file_name, file_type FROM user_files WHERE approved = 0')
        rows = c.fetchall()
        conn.close()

        if not rows:
            bot.answer_callback_query(call.id, "✅ No pending files found!", show_alert=True)
            return

        text = "⏳ <b>Pending Files for Approval:</b>\n\n"
        markup = types.InlineKeyboardMarkup(row_width=1)
        for uid, fname, ftype in rows:
            text += f"👤 User: <code>{uid}</code> | 📄 File: <code>{fname}</code>\n"
            markup.add(types.InlineKeyboardButton(f"Approve: {fname[:15]}", callback_data=f"adm_approve_{uid}_{fname}"))
...
def admin_approve_file(call, target_uid, file_name):
    """Admin approves a file"""
    user_id = call.from_user.id
    if user_id != OWNER_ID and user_id != ADMIN_ID and user_id not in admin_ids:
        bot.answer_callback_query(call.id, "❌ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲!")
        return
    set_file_approved(target_uid, file_name, 1)
    bot.answer_callback_query(call.id, f"✅ Approved {file_name}!")
    try:
        bot.edit_message_text(
            f"✅ <b>File Approved Successfully!</b>\n👤 User ID: <code>{target_uid}</code>\n📄 File: <code>{file_name}</code>",
    ...
    try:
        bot.send_message(
            target_uid,
            f"🎉 <b>Your file <code>{file_name}</code> has been approved by the admin! You can now run it.</b>",
            parse_mode='HTML'
        )
    except:
        pass

def admin_reject_file(call, target_uid, file_name):
    ...
    try:
        if os.path.exists(file_path):
            os.remove(file_path)
        remove_user_file_db(target_uid, file_name)
    except:
        pass
    bot.answer_callback_query(call.id, f"❌ Rejected and deleted {file_name}!")
    try:
        bot.edit_message_text(
            f"❌ <b>File Rejected & Deleted</b>\n👤 User ID: <code>{target_uid}</code>\n📄 File: <code>{file_name}</code>",
            call.message.chat.id, call.message.message_id, parse_mode='HTML'
        )
    except:
        pass
    try:
        bot.send_message(
            target_uid,
            f"❌ <b>Your file <code>{file_name}</code> was rejected and removed by the admin.</b>",
            parse_mode='HTML'
        )
    except:
        pass
...
def stop_all_bots(call):
    ...
    if user_id != OWNER_ID and user_id != ADMIN_ID and user_id not in admin_ids:
        bot.answer_callback_query(call.id, "❌ 𝐀𝐝𝐦𝐢𝐧 𝐨𝐧𝐥𝐲!")
        return
    bot.answer_callback_query(call.id, "🛑 𝐒𝐭𝐨𝐩𝐩𝐢𝐧𝐠 𝐚𝐥𝐥 FREE HOSTING 𝐛𝐨𝐭𝐬...")
    stopped = 0
...
def show_admin_logs(call):
    ...
    try:
        ...
        conn.close()
        if logs:
            text = "📋 <b>FREE HOSTING : RECENT LOGS</b>\n"
            for log in logs:
                text += f"👤 {log[0]} | {log[1]}\n{log[2][:30]}...\n🕐 {log[3][:16]}\n"
        else:
            text = "📋 𝐍𝐨 𝐥𝐨𝐠𝐬."
...
def cleanup_on_exit():
    ...
    logger.info("𝐂𝐥𝐞𝐚𝐧𝐢𝐧𝐠 𝐮𝐩 FREE HOSTING...")
    for script_key in list(bot_scripts.keys()):
        try:
            script_info = bot_scripts[script_key]
            kill_process_tree(script_info)
...
def main():
    ...
    logger.info("=" * 50)
    logger.info("🤖 𝐒𝐭𝐚𝐫𝐭𝐢𝐧𝐠 FREE HOSTING 🇧🇩 𝐁𝐨𝐭...")
    ...
    while True:
        try:
            ...
            logger.error(f"FREE HOSTING, 𝐞𝐫𝐫𝐨𝐫: {e}", exc_info=True)
            time.sleep(5)
